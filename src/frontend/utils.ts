const pieceDictionary: Record<string, ChessPiece> = {
...
export function parseFEN(FEN: string): {
  ...
  const pieces: Array<ChessPiece | null> = [];
...
export function columnNumberToLetter(col: number) {
  switch (col) {
    case 1:
      return "a";
    case 2:
...
export function columnLetterToNumber(col: string) {
  switch (col) {
    case "a":
      return 0;
    case "b":
...
export function parseChessCoordinate(input: string): { row: number; col: number } {
  const col = columnLetterToNumber(input[0]);
  const row = Math.abs(parseInt(input[1]) - 1 - 7);
  return { row, col };
}
export function getPieceLetter(piece: ChessPieceType) {
  switch (piece) {
    case "king":
      return "K";
    case "queen":
...
export function getPieceType(letter: string): ChessPieceType {
  switch (letter) {
    case "K":
      return "king";
    case "Q":
